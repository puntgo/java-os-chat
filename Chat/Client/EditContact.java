...
class EditContact extends Dialog implements ActionListener
{
	JFrame jf;
	JPanel newPanel;
	...
	EditContact(JFrame j,MySocket ms,String Email)
	{
		super(j,"Edit Contact",true);
		jf = j;
		s = ms;
		UserEmail = Email;
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if(btn.equals(btnOK))
		{
			...
			String str = "EDTC"+(char)27+Name+(char)27+NickName+(char)27+Email+(char)27+Address+(char)27+Phone;
			s.Write(str);
			str = s.Read();
			...
			if(response.equals("ERR"))
			{
				...
				{
					new MessageBox(jf,"New Contact Failed","A contact with specified Email ID does not exists in your contact list.",1);
				}
				else
				{
